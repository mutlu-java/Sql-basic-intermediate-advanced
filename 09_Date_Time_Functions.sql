...
SELECT
    ...
    -- DATENAME Examples ** always returns string value .
...
-- finding old total sales using union and subquery
SELECT 
...
)t GROUP BY DATETRUNC(year,CreationTime)
...
*/
SELECT * --MONTH(orderdate) returns integer
FROM Sales.Orders
...
SELECT 
    ...
    AVG(DATEDIFF(day,OrderDate,ShipDate)) AS AvgShipment
FROM Sales.Orders   
GROUP BY  DATENAME(month,OrderDate) 
------
SELECT
